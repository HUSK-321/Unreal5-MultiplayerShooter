...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if(bWasSuccessful)
	{
		if(GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue, FString::Printf(TEXT("Session successfullly maked : %s"), *SessionName.ToString()));
		}
	...
	else
	{
		if(GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Red, FString(TEXT("Failed to make session")));
		}
	}
}
...
void AMenuSystemCharacter::OnFindSessionComplete(bool bWasSuccessful)
{
	...
	for(auto Result : SessionSearch->SearchResults)
	{
		...
		auto UserName = Result.Session.OwningUserName;
		FString MatchType;
